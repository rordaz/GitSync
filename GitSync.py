...
if applicationSettings["job_schedule_frequency"] == Periodicity.MINUTE.value:
    schedule.every(applicationSettings["job_schedule_factor"]).minutes.do(gt.GitSync,repositoriesDict)
elif applicationSettings["job_schedule_frequency"] == Periodicity.HOURLY.value:
    schedule.every(applicationSettings["job_schedule_factor"]).hours.do(gt.GitSync,repositoriesDict)
elif applicationSettings["job_schedule_frequency"] == Periodicity.DAILY.value:
    schedule.every(applicationSettings["job_schedule_factor"]).days.do(gt.GitSync,repositoriesDict)


# if applicationSettings["job_schedule_frequency"] == "minute":
#     schedule.every(applicationSettings["job_schedule_factor"]).minutes.do(gt.GitSync(repositoriesDict))
